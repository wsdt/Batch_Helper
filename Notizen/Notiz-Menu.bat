...
cls
echo.
title SMART SPEED %version% - Note Menu
set mode=SS-Note Menu %version%
...
echo.
echo * (1) Neue Notiz * (2) Notiz erledigt * (3) Notiz editieren * (0) NICHTS *
set /p load=--- 
if "%load%"=="1" call "%cd%\Notiz-Eintragen.bat"&goto endnote
if "%load%"=="2" call "%cd%\Notiz-Erledigt.bat"&goto endnote
if "%load%"=="3" call "%cd%\Notiz-Edit.bat"&goto endnote
if not "%load%"=="0" echo.&echo -- ERROR 404 --&echo Eingabe existiert nicht!&echo (SMART SPEED wird fortgesetzt..)&timeout 15 >Nul
:endnote
